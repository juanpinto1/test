...
app.put("/cafes/:id", (req, res) => {
    ...
    const cafeIndexFound = cafes.findIndex((p) => p.id == cafeId);
    if (cafeIndexFound >= 0) {
...
app.delete("/cafes/:id", (req, res) => {
    ...
    if (jwt) {
        ...
        if (cafeIndexFound >= 0) {
            ...
            res.status(200).send(cafes); // Cambia el código de estado a 200
        } else {
            res.status(404).send({ error: "No se encontró ningún café con ese id" }); // Cambia el código de estado a 400
        }
    } else {
